...
def legacyentry():
    ...
    parser_info = subparsers.add_parser(
        'info', help='Show basic Hive blockchain info')
    ...
    parser_powerdown = subparsers.add_parser(
        'powerdown',
        help='Power down (start withdrawing HIVE from HIVE POWER)')
    ...
    parser_convert = subparsers.add_parser(
        'convert',
        help='Convert HBD to HIVE (takes 3.5 days to settle)')
    parser_convert.set_defaults(command="convert")
    parser_convert.add_argument(
        'amount', type=float, help='Amount of HBD to convert')
    parser_convert.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    parser_sell.add_argument(
        'price',
        type=float,
        help='Limit sell price denoted in (HBD per HIVE)')
    parser_sell.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    """
        Command "reblog"
    """
    parser_reblog = subparsers.add_parser(
        'reblog', help='Reblog an existing post')
    parser_reblog.set_defaults(command="reblog")
    parser_reblog.add_argument(
        'identifier',
        type=str,
        help='author/permlink-identifier of the post to reblog')
    parser_reblog.add_argument(
        '--account',
        type=str,
        required=False,
    ...
    if args.command == "set":
        ...
        if args.unsafe_import_key:
            ...
            while True:
                ...
                if len(installed_keys) == 1:
                    name = hive.commit.wallet.getAccountFromPublicKey(
                        installed_keys[0])
                    print("=" * 30)
        ...
        print_json(hive.commit.convert(
            args.amount,
            account=args.account,
        ))
        ...
        print_json(
            hive.commit.update_memo_key(args.key, account=args.account))
        ...
        print_json(
            hive.commit.approve_witness(args.witness, account=args.account))

    elif args.command == "disapprovewitness":
        print_json(
            hive.commit.disapprove_witness(
        ...
        props = {
            "account_creation_fee":
                str(Amount("%f HIVE" % args.account_creation_fee)),
            "maximum_block_size":
                args.maximum_block_size,
            "hbd_interest_rate":
                int(args.hbd_interest_rate * 100)
        }
        print_json(
            hive.commit.witness_update(
