...
from .instance import shared_hived_instance, hve
...
class Blockchain(object):
    ...
    Args:
        hived_instance (Hived): Hived() instance to use when accessing a RPC
        mode (str): `irreversible` or `head`. `irreversible` is default.
    """

    def __init__(self, hived_instance=None, mode="irreversible"):
    ...
    def info(self):
        ...
        """
        return self.hive.get_dynamic_global_properties()

    def config(self):
        return self.hive.get_config()
    ...
    def get_current_block(self):
        """ This call returns the current block
        """
        return self.hive.get_block(self.get_current_block_num())
    ...
    def stream_from(self,
        ...
        operations, return raw, unedited blocks as provided by hived. This
       ...
       """
        ...
        block_interval = self.config().get("HIVE_BLOCK_INTERVAL")
        ...
        while True:
            ...
            for block_num in range(start_block, head_block + 1):
                ...
                if full_blocks:
                    ...
                    yield self.hive.get_ops_in_block(block_num, False)
                else:
                    for ops in self.hive.get_ops_in_block(block_num, False):
                        yield ops
    ...
    def reliable_stream(self,
        ...
        until getting a response from hived, allowing catching up after
        server downtime.

        Warnings: To ensure reliability, this method does some weird
        none-standard things with the hived client
        ...
        provided, will attempt to query hived for this value
        ...
        timeout (int): Time to wait on response from hived before assuming
        ...
        def get_reliable_client(_timeout):
            ...
            return hve.hived.Hived(
                nodes=self.hive.nodes,
                retries=1,
                timeout=_timeout,
                re_raise=True)
        ...
        def get_reliable_block_interval(_client):
            return reliable_query(_client, 'get_config',
                                  'database_api').get('HIVE_BLOCK_INTERVAL')
        ...
        for ops in self.stream_from(*args, **kwargs):
            ...
            for event in events:
                ...
                if not filter_by or op_type in filter_by:
                    # return unmodified hived output
    ...
    def get_all_usernames(self, *args, **kwargs):
        ...
        _ = args, kwargs
        warnings.warn(
            'Blockchain.get_all_usernames() is now Hived.get_all_usernames().'
        )
        return self.hive.get_all_usernames()
