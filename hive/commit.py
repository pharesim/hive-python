...
class Commit(object):
    ...
    def post(self,
        ...
        if comment_options or beneficiaries:
            options = keep_in_dict(comment_options or {}, [
                'max_accepted_payout', 'percent_hive_dollars', 'allow_votes',
            ...
            comment_op = operations.CommentOptions(
                **{
                    ...
                    "percent_hive_dollars":
                        int(options.get("percent_hive_dollars", 10000)),
                    "allow_votes":
    ...
    def create_account(
        ...
        if password:
            ...
            memo_pubkey = PublicKey(
                memo_key, prefix=self.hived.chain_params["prefix"])
        else:
            raise ValueError(
        ...
        s = {
            'creator': creator,
            'fee': '%s HIVE' % (delegation_fee_hive or required_fee_hive),
    ...
    def interest(self, account):
        ...
        interest_rate = self.hived.get_dynamic_global_properties()[
                            "hbd_interest_rate"] / 100  # percent
        interest_amount = (interest_rate / 100) * int(
            int(account["hbd_seconds"]) / (60 * 60 * 24 * 356)) * 10 ** -3
    ...
    def disallow(self,
        ...
        try:
            pubkey = PublicKey(
                foreign, prefix=self.hived.chain_params["prefix"])
            affected_items = list(
        ...
        op = operations.CommentOptions(
            **{
                ...
                "max_accepted_payout":
                    options.get("max_accepted_payout", default_max_payout),
                "percent_hive_dollars":
                    options.get("percent_hive_dollars", 100) * HIVEIT_1_PERCENT,
                "allow_votes":
                    options.get("allow_votes", True),
                "allow_curation_rewards":
                    options.get("allow_curation_rewards", True),
