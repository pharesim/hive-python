...
from hivebase.exceptions import (InsufficientAuthorityError, MissingKeyError,
                                  InvalidKeyFormat)
from hivebase import operations
from hivebase.operations import Operation
from hivebase.transactions import SignedTransaction, fmt_time_from_now, \
    get_block_params
...
class TransactionBuilder(dict):
    ...
    def __init__(self,
        ...
        self.wallet = wallet_instance or Wallet(self.hived)
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[permission]["account_auths"]:
                    auth_account = Account(
                        authority[0], hived_instance=self.hived)
    ...
    def broadcast(self):
        ...
        try:
            self.hived.broadcast_transaction(self.json())
        except Exception as e:
            raise e
    ...
    def addSigningInformation(self, account, permission):
        ...
        """
        accountObj = Account(account, hived_instance=self.hived)
        ...
        for account_auth in authority["account_auths"]:
            account_auth_account = Account(
                account_auth[0], hived_instance=self.hived)
        ...
        self["blockchain"] = self.hived.chain_params
