...
from hivebase import bip38
from hivebase.account import PrivateKey
from hivebase.exceptions import (InvalidWifError, WalletExists)
...
class Wallet:
    ...
    def __init__(self, hived_instance=None, **kwargs):
        ...
        # Prefix
        if self.hived:
            self.prefix = self.hived.chain_params["prefix"]
        else:
        ...
        if "keys" in kwargs:
            ...
            """ If no keys are provided manually we load the SQLite
                keyStorage
            """
            from hivebase.storage import (keyStorage, KeyEncryptionKey)
    ...
    def getOwnerKeyForAccount(self, name):
        ...
        if "owner" in Wallet.keyMap:
            ...
            account = self.hived.get_account(name)
            if not account:
                return
